...
function readConfig(){
    ...
    do
        ...
        CONFIG_ARY=("${CONFIG_ARY[@]}" "$value")
    done < "config.txt"
    
    debugLog  "CONFIG_ARY: ${CONFIG_ARY[@]}"
    eval $ret="'${CONFIG_ARY[@]}'"
    
}

function getFileNameVersion(){
    local FILENAME_VERSION=333
    local abc=("$abc" "$1")
    debugLog  "CONFIG_CONTENT: ${abc[@]}"
...
#call func : read config to CONFIG_CONTENT
readConfig CONFIG_CONTENT


debugLog  "CONFIG_CONTENT: ${CONFIG_CONTENT[@]}"
getFileNameVersion ${CONFIG_CONTENT[@]}
TEMP=$?
debugLog "TEMP: $TEMP"

#doSyncAPP
#deployAPP  CONFIG_CONTENT
